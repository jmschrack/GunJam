...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class Cover : NavMeshMovement
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            Debug.DrawLine(transform.position, coverPoint);
        ...
        private Vector3 Target()
        {
            ...
            while (raycastCount < maxRaycasts.Value) {
                ...
                if (Physics.Raycast(ray, out hit, maxCoverDistance.Value, availableLayerCovers)) {
                    ...
                    if (hit.collider.Raycast(new Ray(playerT.position, hit.point - playerT.position), out hit, Mathf.Infinity)) {
                        coverPoint = hit.point;
                        coverOwner = hit.collider.gameObject.GetComponent<CoverOwner>();
                        if(coverOwner.owner == null)
                        {
                            coverOwner.owner = this.gameObject;
